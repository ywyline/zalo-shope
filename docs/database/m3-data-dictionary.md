...
> 状态：M3.1 冻结；M3.2 数据库基础、M3.3 仓库/库存、M3.4 搜索、M3.5 促销/报价与 M3.6 购物车已实施；M3.7 阶段收口进行中
>
> 日期：2026-07-22
...
M3.4 的应用投影在商品发布/停用以及品牌、类目可见内容变更的同一商城事务内同步；检索文档组合多个有界目录字段，因此不沿用 100 字查询输入上限。受审 `search:rebuild` 命令按商城在单一事务内删除并重建派生文档，不修改商品、库存、历史或热门词事实。公开搜索使用 Redis 固定窗口短期限流，限流键只含环境和经 HMAC 处理的来源地址。
...
- 商城归属不可变触发器覆盖本字典所有表。库存流水、`promotion_operations` 和发布版本额外触发器拒绝 UPDATE/DELETE；预留终态触发器拒绝恢复或改换终态。
...
- 仅无真实 M3 数据的 local/test 环境允许人工执行 `down.sql`。产生库存流水、促销命令记录、发布版本或购物车数据后只允许向前修复；应用回滚到 M2 时保留 M3 schema 和数据。
