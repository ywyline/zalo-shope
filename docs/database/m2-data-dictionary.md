...
- 买家目录 API 不新增数据库表或迁移，复用 M2.2 关系、RLS 和不可变发布版本；每次公开读取先用 `X-Store-Code` 解析启用商城，再在 `withStoreTransaction` 的商城上下文内查询。
- 首页只读取当前 `PUBLISHED` 页面版本、启用且处于展示时间窗的模块；内部目标转换为同商城业务编码，外部目标再次执行 HTTPS 与主机白名单校验，不向客户端暴露内部目标 UUID。
- 品牌、类目和商品仅返回启用/已发布记录；请求语言缺失时回退越南语并同时返回 `requested_locale` 与 `resolved_locale`。没有越南语基线的脏数据不会进入买家响应。
- 商品列表用白名单排序和严格校验的结构化游标；类目过滤支持根类目覆盖二级类目。商品价格只由启用 SKU 的整数 VND 生成展示区间，成本价、内部 ID 和库存信息不进入买家 API。
- READY 媒体通过对象存储适配器生成五分钟签名读取 URL，并返回本地化替代文本、宽高和过期时间；对象键和长期存储地址不暴露给买家端。

## 14. M2.7 实施说明

- M2.7 不新增表或迁移。UTF-8 CSV 导入按商品分组，每个商品及全部 SKU 使用一个商城 RLS 事务创建；不同商品可部分成功并返回逐行报告，`dry_run` 不写数据库。
- 导入模板只接受业务编码、三语草稿内容、整数 VND、重量和规格选项；明确不接受 `store_id`、库存、状态、媒体、合规审批或发布字段。商城归属只来自认证后的可信上下文。
- 文件限制为 2 MiB、2,000 个数据行和每商品 500 个 SKU；严格校验 UTF-8、固定表头、CSV 引号、字段长度、规格选项、同商城引用与业务编码唯一性。
- M2.8.3 不新增表或迁移。XLSX 导入复用相同的 20 列与唯一行到领域输入转换，并在解析前限制 ZIP 条目、声明解压大小、压缩比和路径；只允许一个名为 `products` 的可见工作表，拒绝公式、宏和外链。
- 商品 XLSX 导出是一行一个 SKU 的同步只读投影，无 SKU 草稿保留空 SKU 行；最多 10,000 行，只查询当前授权商城。导出不包含成本价、内部/商城 ID、合规证件或媒体对象键，也不产生数据库或对象存储记录。
- 发布版本列表和详情读取均绑定同商城 RLS。数据库快照保持完整且不可变；`store.catalog.read` 的版本详情响应递归移除 `costPriceVnd`/`cost_price_vnd`，避免通过历史版本旁路读取成本价。
- 批量停用和主类目移动最多 100 个不重复商品，逐项使用独立事务、`expected_version` 和审计。停用保留历史版本；移动仅允许草稿/已下架商品，已有 SKU/属性时禁止跨属性模板移动。
