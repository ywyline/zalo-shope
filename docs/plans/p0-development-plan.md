...
> 状态：已批准，M0 已完成，M1 实施完成但验收有保留；M2.1-M2.8.4 已实施并完成自动化收口；M3.1-M3.6 已完成，下一阶段为 M3.7；外部上线前置条件有保留
>
> 版本：0.1
>
> 日期：2026-07-17
...
迁移/回滚：库存初始余额和流水在同一受审迁移/导入事务中建立；不可直接回滚已发生的库存流水，使用反向调整。M3.5 新增的 `promotion_operations` 强制 RLS 且只追加，产生命令记录后只允许向前修复。
