...
> 状态：已批准，M0 已完成，M1 实施完成但验收有保留；M2.1-M2.7 已实施但总体验收有保留
>
> 版本：0.1
>
> 日期：2026-07-17
...
M2 批准与实施记录（2026-07-18 至 2026-07-19）：用户明确接受 Zalo Mini App 真机验收作为结转风险并批准 `docs/plans/m2-implementation-plan.md`。M2.1 已冻结字段字典、状态、权限和 OpenAPI 草案；M2.2 已交付 Prisma 关系模型、扩展迁移、29 张商城表的 RLS/复合约束/不可变保护、权限与目录基础种子；M2.3 已交付品牌、两级类目和属性模板版本后台 API；M2.4 已交付商品草稿、SKU、真实 local/test S3 兼容上传、合规分权审核、上架门禁和不可变发布快照；M2.5 已交付装修后台 API、不可变页面发布和三语可视化内容工作台；M2.6 已交付公开买家目录 API、短期签名媒体读取、双商城共用组件树的三语移动端 UI，并完成真实浏览器双主题、窄屏/桌面、SKU 与降级状态验收；M2.7 已交付受限 UTF-8 CSV 导入、商品版本读取、批量停用/移动与收口文档。最新 63 项单元测试、47 项全量集成测试和全部静态/构建门禁通过；XLSX、商品属性值编辑 API、目录管理端 UI 和商品导出仍是 M2 总体验收保留项。不得自动进入 M3。
