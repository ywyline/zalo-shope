...
> 仓库门禁通过；B4 default-disabled repository implementation + local/test validation 也已完成且适用
> 仓库门禁通过；B2/B2b、B5-B7、M6.3、UI 与生产启用未完成或未授权并保持失败关闭；生产策略、TTL、对象存储、真实供应商、
...
复核和 SYSTEM 寄回到期。B3/B4 repository-local-test 验收已完成。B2a 不改写政策 RLS，保留会员历史
政策读取；B3/B4 也不提供返件物流、真实退款/COD、验收、库存恢复、换货履约或 UI。只读摘要、政策
控制面、凭证链路和默认关闭写命令存在，仍不能解释为完整售后产品或生产可用。
完整 B2b、B5-B7、生产政策与 enforcement、TTL、对象存储、真实供应商、部署和 rollout 均
