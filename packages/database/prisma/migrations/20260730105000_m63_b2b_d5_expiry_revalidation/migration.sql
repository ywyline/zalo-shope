-- D5 forward repair: a lifecycle writer can hold the evidence row lock while
-- the final authorization transaction waits. Recheck bearer, session, signed
-- URL and ordinary-access deadlines only after the evidence lock is acquired.
BEGIN;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'M6.3-B2b-D5 expiry revalidation requires the restricted guard role'
      USING ERRCODE = '55000';
  END IF;
  IF EXISTS (
    SELECT 1
...
BEGIN
  ...
  IF post_lock_now >= caller_token_expires_at
     ...
     OR post_lock_now >= target_url_expires_at
