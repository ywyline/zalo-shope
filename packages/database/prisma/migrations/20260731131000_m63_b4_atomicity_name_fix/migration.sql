-- Avoid PL/pgSQL ambiguity between the local aggregate identifier and table columns.
...
DECLARE target_after_sale_id uuid;
...
BEGIN
  IF TG_TABLE_NAME = 'after_sale_operations' THEN
    ...
    target_after_sale_id := NEW.after_sale_id;
  ELSE
    IF NEW.event NOT IN (
  ...
  WHERE operation_row.store_id = NEW.store_id
    ...
    AND operation_row.after_sale_id = target_after_sale_id;
  SELECT pg_catalog.count(*) INTO linked_count
  FROM public.after_sale_transitions transition_row
  WHERE transition_row.store_id = NEW.store_id
    AND transition_row.after_sale_id = target_after_sale_id
    AND transition_row.operation_id = target_operation_id;
  SELECT pg_catalog.count(*) INTO audit_count
  FROM public.audit_logs audit
  WHERE audit.store_id = NEW.store_id
    AND audit.target_type = 'after_sale'
    AND audit.target_id = target_after_sale_id::text
    AND audit.after_data->>'operation_id' = target_operation_id::text
