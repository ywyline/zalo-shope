-- Forward-only repair for M6.2 defects discovered after the lock-order
-- closeout: definer capacity guards must fail closed before protected reads,
-- and only approved/side-effected cases reserve order-level approval capacity.
...
BEGIN
  ...
  WHERE other_sale.store_id = NEW.store_id
    ...
    AND (
      (
        other_sale.status NOT IN ('PENDING_REVIEW','REJECTED','CANCELLED')
        AND NOT (
          other_sale.status = 'REVIEW_REQUIRED'
          AND other_sale.legacy_policy_review
          AND other_sale.review_resume_status IS NULL
        )
      )
      OR EXISTS (
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'after-sale item capacity requires the current scoped actor'
      USING ERRCODE = '42501';
  END IF;
