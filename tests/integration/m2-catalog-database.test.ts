  ...
  it('protects activated templates, reviewed compliance and published versions from mutation', async () => {
    ...
    await withStoreTransaction(
      ...
      async (transaction) => {
        ...
        await transaction.$executeRaw`INSERT INTO compliance_requirements (id, store_id, code, industry, document_type, version, updated_at) VALUES (${requirementId}::uuid, ${BEAUTY_STORE_ID}::uuid, ${`requirement-${requirementId.slice(0, 8)}`}, 'BEAUTY', 'TEST_DOCUMENT', 1, now())`;
